...
#`${TEsorter}TEsorter $cds -p $threads 2>/dev/null`;
#
#
# Only in plants TEs
`${TEsorter}TEsorter -db rexdb-plant --hmm-database rexdb-plant $cds -p $threads 2>/dev/null`;
#
#
#
#
...
`cat $cds.rexdb-plant.cls.tsv > $cds.TE.list`;
...
`${repeatmasker}RepeatMasker -e ncbi -pa $threads -q -no_is -norna -nolow -div 40 -cutoff 225 -lib $cds.rmTE $rawlib 2>/dev/null`;
